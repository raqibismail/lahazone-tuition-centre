<?php
    ...
    $stdperf = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM stud_performance WHERE STUD_ID = '".$par_type['STUD_ID']."'"));
?>
...
<body>
    ...
    <div class=" p-4">
        ...
        <div class="table-responsive mt-1">    
            <table class="table table-bordered text-center text-dark font-weight-bolder" style="table-layout: fixed;">
                <tr>
                    <th>STUDENT NAME</th>
                    <th>COURSE</th>
                    <th>GRADE</th>
                ...
                while($row = mysqli_fetch_assoc($result)){
                    $query2 = "SELECT * FROM stud_performance WHERE STUD_ID = '".$row['STUD_ID']."'";
                    $result2 = mysqli_query($conn, $query2);
                    while($row2 = mysqli_fetch_assoc($result2)){
                        $stud = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM student WHERE STUD_ID = '".$row['STUD_ID']."'"));    
                        $course = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM staff WHERE STAFF_ID = '".$row2['STAFF_ID']."'"));    
                    ?>
                <tr>
                    <td style="word-wrap: break-word"><?php echo $stud['STUD_NAME'] ?></td>
                    <td style="word-wrap: break-word"><?php echo $course['COURSE_ID'] ?></td>
                    <td style="word-wrap: break-word"><?php echo $row2['GRADE'] ?></td>
                    <td style="word-wrap: break-word"><?php echo $row2['GRADE_DESC']?></td>
                </tr>
                <?php }} ?>
