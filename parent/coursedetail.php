    ...
    <div class="container">
        ...
        <table class="table table-bordered text-center"> 
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                $query2 = "SELECT * FROM stud_course WHERE STUD_ID =  '" . $row['STUD_ID'] . "' ";
                $result2 = mysqli_query($conn, $query2);
                while ($row2 = mysqli_fetch_assoc($result2)) {
                    $student = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM student WHERE STUD_ID = '" . $row2['STUD_ID'] . "'"));
                    $course = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM course WHERE COURSE_ID = '" . $row2['COURSE_ID'] . "' "));

                    // Output the data for each row
                    echo "<tr>";
                    echo "<td>" . $student['STUD_NAME'] . "</td>";
                    echo "<td>" . $course['COURSE_ID'] . "</td>";
                    echo "<td>" . ucwords($course['COURSE_NAME']) . "</td>";
                    // echo "<td>";
