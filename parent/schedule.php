...
<body>
   ...
   <div class="jumbotron jumbotron-fluid jumbotron-sm position-relative" style="margin-bottom: 0px ">
    ...
    <div class="container">
        ...
        $stud = mysqli_query($conn, "SELECT * FROM stud_par WHERE PAR_ID = '" . $user['PAR_ID'] . "'");
            while($row = mysqli_fetch_assoc($stud)){      
            $query2 = "SELECT COURSE_ID FROM stud_course WHERE STUD_ID =  '".$row['STUD_ID']."' ";
            $result2 = mysqli_query($conn, $query2);

            while ($row2 = mysqli_fetch_assoc($result2)) {

                $query = "SELECT * FROM schedule WHERE COURSE_ID = '" . $row2['COURSE_ID'] . "' ";
                $result = mysqli_query($conn, $query);

                // Output the data for each row
                while ($row3 = mysqli_fetch_assoc($result)) {
                    $stu = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM STUDENT WHERE STUD_ID = '" . $row['STUD_ID'] . "'"));
                    echo "<tr>";
                    echo "<td>" . $row['STUD_ID'] . "</td>";
                    echo "<td>" . $stu['STUD_NAME'] . "</td>";
                    echo "<td>" . $row3['COURSE_ID'] . "</td>";
                    echo "<td>" . $row3['CLASS_ID'] . "</td>";
                    echo "<td>";
                    echo "<a href=' " . $row3['CLASS_ATTAND'] . "'> " . $row3['CLASS_ATTAND'] . " </a>";
                    echo "</td>";
                    echo "<td>" . $row3['CLASS_DATE'] . "</td>";
                    // echo "<td>";
