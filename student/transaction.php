   ...
   <div class="jumbotron jumbotron-fluid jumbotron-sm position-relative" style="margin-bottom: 0px ">
    ...
    <div class="container">
        ...
        <?php
            ...
            $query2 = "SELECT * FROM invoice WHERE STUD_ID =  '".$user['STUD_ID']."' ";
            $result2 = mysqli_query($conn, $query2);

            while ($row2 = mysqli_fetch_assoc($result2)) {

                $query = "SELECT * FROM payment WHERE PAY_ID = '" . $row2['PAY_ID'] . "' ";
                $result = mysqli_query($conn, $query);

                // Output the data for each row
                while ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr>";
                    if(strcasecmp($row['PAY_STATUS'],"Not Paid")== 0)
                    {
                        echo "<td><a href='paymentpage.php?id=". $row2['PAY_ID']."'>". $row2['PAY_ID']."</a></td>";
                        echo "<td>" . $user['STUD_ID'] . "</td>";
                        echo "<td>" . $user['STUD_NAME'] . "</td>";
                        echo "<td> RM" . $row['PAY_FEE'] . "</td>";
